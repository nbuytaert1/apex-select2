...
begin
  ...
  if (l_select_list_type = 'TAG') then
    ...
    if (l_lov.exists(gc_lov_group_col)) then
      for i in 1 .. l_lov(gc_lov_display_col).count loop
        if (not optgroup_exists(laa_optgroups, l_lov(gc_lov_group_col)(i))) then
          sys.htp.p('<optgroup label="' || l_lov(gc_lov_group_col)(i) || '">');
          for j in 1 .. l_lov(gc_lov_display_col).count loop
            if (l_lov(gc_lov_group_col)(i) = l_lov(gc_lov_group_col)(j)) then
              apex_plugin_util.print_option(
                p_display_value => l_lov(gc_lov_display_col)(j),
                p_return_value  => l_lov(gc_lov_return_col)(j),
                p_is_selected   => apex_plugin_util.is_equal(l_lov(gc_lov_return_col)(j), p_value),
          ...
          laa_optgroups(i) := l_lov(gc_lov_group_col)(i);
        end if;
      end loop;
    else
      for i in 1 .. l_lov(gc_lov_display_col).count loop
        apex_plugin_util.print_option(
          ...
          p_is_selected   => apex_plugin_util.is_equal(l_lov(gc_lov_return_col)(i), p_value),
          p_attributes    => p_item.element_option_attributes,
          p_escape        => p_item.escape_output
        );
      end loop;
  ...
  l_onload_code := l_onload_code || '
    apex.widget.selectList(
      $("#' || p_item.name || '"),
      {' ||
        add_js_attr('nullValue', p_item.lov_null_value, true) ||
        add_js_attr('dependingOnSelector', apex_plugin_util.page_item_names_to_jquery(p_item.lov_cascade_parent_items), true) ||
        add_js_attr('pageItemsToSubmit', apex_plugin_util.page_item_names_to_jquery(p_item.ajax_items_to_submit), true) ||
        add_js_attr('optimizeRefresh', boolean_to_string(p_item.ajax_optimize_refresh), false) ||
        add_js_attr('ajaxIdentifier', apex_plugin.get_ajax_identifier, true, false) || '
      }
    );';
