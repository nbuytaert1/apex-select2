...
begin
  if l_lazy_loading is not null then
    ...
    if (l_select_list_type != 'TAG' or
      ...
      if p_include_key then
        for i in 1 .. l_lov(gco_lov_display_col).count loop
          l_tags_option := l_tags_option || '"' || htf.escape_sc(l_lov(gco_lov_display_col)(i)) || '",';
        end loop;
      ...
      for i in 1 .. l_lov(gco_lov_display_col).count loop
        l_tags_option := l_tags_option || '{' ||
                                             apex_javascript.add_attribute('id', htf.escape_sc(l_lov(gco_lov_return_col)(i)), false, true) ||
                                             apex_javascript.add_attribute('text', htf.escape_sc(l_lov(gco_lov_display_col)(i)), false, false) ||
                                          '},';
      end loop;
  ...
  begin
    ...
    l_code := '
      $("' || l_item_jq || '").select2({' ||
        ...
        add_js_attr('maximumSelectionLength', l_max_selection_size) ||
    ...
    if l_lazy_loading is not null then
      if p_value is not null then
        ...
        for i in 1 .. l_selected_values.count loop
          l_json := l_json ||
            '{' ||
               apex_javascript.add_attribute('R', htf.escape_sc(l_selected_values(i)), false, true) ||
               apex_javascript.add_attribute('D', htf.escape_sc(l_display_values(i)), false, false) ||
            '},';
        end loop;
    ...
    if (l_select_list_type = 'TAG' and l_lazy_loading is not null) then
      l_code := l_code || '
        createTag: function(item) {
          if ($.trim(item.term).length != 0) {
            return {R: item.term, D: item.term};
          }
        },';
    end if;
...
begin
  ...
  apex_javascript.add_library(
    p_name      => 'js/select2.full.min',
    p_directory => p_plugin.file_prefix,
    p_version   => null
  );
  apex_javascript.add_library(
    p_name      => 'js/select2-apex',
    p_directory => p_plugin.file_prefix,
    p_version   => null
  );
  apex_css.add_file(
    p_name      => 'css/select2.min',
    p_directory => p_plugin.file_prefix,
    p_version   => null
  );
  if l_look_and_feel = 'UT' then
    apex_css.add_file(
      p_name      => 'css/select2-ut',
      p_directory => p_plugin.file_prefix,
      p_version   => null
    );
  end if;
  ...
  if (l_select_list_type = 'TAG' or l_lazy_loading is not null) then
    null; -- TO DO: default selections
  else
    htp.p(get_options_html(p_item, p_plugin, p_value));
  end if;
  ...
  if l_drag_and_drop_sorting is not null then
    ...
    if l_apex_version = '4.1' then
      apex_javascript.add_library(
        p_name      => 'js/jquery-ui.custom.min',
        p_directory => p_plugin.file_prefix,
        p_version   => null
      );
...
begin
  if apex_application.g_x03 = 'LAZY_LOAD' then
    ...
    while l_index < l_last_row loop
      ...
      l_json := l_json ||
        '{' ||
           apex_javascript.add_attribute('R', htf.escape_sc(l_lov(gco_lov_return_col)(l_index)), false, true) ||
           apex_javascript.add_attribute('D', htf.escape_sc(l_lov(gco_lov_display_col)(l_index)), false, false) ||
        '},';
        l_index := l_index + 1;
    end loop;
    ...
    if l_select_list_type = 'TAG' then
      htp.p(get_tags_option(p_item, false));
    else
      htp.p(get_options_html(p_item, p_plugin, ''));
    end if;
  end if;
